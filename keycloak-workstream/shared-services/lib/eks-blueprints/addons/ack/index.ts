import { IManagedPolicy, ManagedPolicy, Policy, PolicyStatement } from "aws-cdk-lib/aws-iam";
import { Construct } from "constructs";
import { merge } from "ts-deepmerge";
import { ClusterInfo, Values } from "@aws-quickstart/eks-blueprints/dist/spi";
import "reflect-metadata";
import { createNamespace, setPath, supportsX86 } from "@aws-quickstart/eks-blueprints/dist/utils";
...
export interface AckAddOnProps extends HelmAddOnUserProps {
  ...
  /**
   * To use a Managed IAM Policies for the ack controller
   * @enum unsafe - Will use the service mapping full permission policy for the target service
   
   */
  managedPolicies?: [IManagedPolicy] | "unsafe";
  /**
...
export class AckAddOn extends HelmAddOn {
  ...
  deploy(clusterInfo: ClusterInfo): Promise<Construct> {
    ...
    if (this.options.managedPolicies && this.options.managedPolicies instanceof Array) {
      this.options.managedPolicies.forEach(policy => sa.role.addManagedPolicy(policy));
    }
    if (this.options.inlinePolicyStatements && this.options.inlinePolicyStatements.length > 0) {
      sa.role.attachInlinePolicy(
        new Policy(cluster.stack, `${this.options.chart}-inline-policy`, {
...
function populateDefaults(defaultProps: AckAddOnProps, props?: AckAddOnProps): AckAddOnProps {
  ...
  tempProps.inlinePolicyStatements =
    tempProps.inlinePolicyStatements ?? serviceMappings[tempProps.serviceName!]?.inlinePolicyStatements;
  ...
  if (tempProps.managedPolicies && tempProps.managedPolicies === "unsafe") {
    const managedPolicyName = serviceMappings[tempProps.serviceName!]?.managedPolicyName;
    tempProps.managedPolicies = [ManagedPolicy.fromAwsManagedPolicyName(managedPolicyName!)];
  }
