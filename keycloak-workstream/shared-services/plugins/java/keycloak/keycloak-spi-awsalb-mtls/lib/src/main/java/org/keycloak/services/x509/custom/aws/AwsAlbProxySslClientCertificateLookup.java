...
public class AwsAlbProxySslClientCertificateLookup extends AbstractClientCertificateFromHttpHeadersLookup {

    private static final Logger log = Logger.getLogger(AwsAlbProxySslClientCertificateLookup.class);
    ...
    public AwsAlbProxySslClientCertificateLookup(String sslClientCertHttpHeader,
        ...
        if (!this.isTruststoreLoaded) {
            log.warn("Keycloak Truststore is null or empty, but it's required for AWS ALB x509cert-lookup provider");
            log.warn(
    ...
    /**
     * Decoding end user certificate, including URL decoding.
     */
    @Override
    protected X509Certificate decodeCertificateFromPem(String pem) throws PemException {
        ...
        log.debug(MessageFormat.format("Restored Certificate - \n{0}", pem));
        ...
        if (clientCert != null) {
            ...
            if (certChain == null || certChain.length == 0) {
                ...
                for (X509Certificate caCert : certChain) {
                    chain.add(caCert);
                    log.debugf("Rebuild user cert chain DN : %s", caCert.getSubjectX500Principal());
                }
            }
        }
     ...
     * we are rebuilding here the end user certificate chain with Keycloak
     * truststore.
     * <br>
