...
def validator(bucket, key, receipt_handle,approved_filetypes, mime_mapping):
    logging.info('Validating Zip File Contents')
    ext = ''
    file_type = ''
    mime = ''
    new_tags = {}
    ...
    for f in files:
        ...
        mime = file_data[0][3]
        logging.info(f'Attempting to validate {f}')
        try:
            if file_type.endswith(ext):
                logging.info(f'File Type: {file_type} matches File Extension {ext}')
                if file_type in approved_filetypes:
                    logging.info(f'File Type {file_type} is an approved Type')
    ...
    if valid:
        logging.info('Validating Zip File')
        try:
            ...
            if zip_file_type.endswith(ext):
                logging.info(f'File Type: {zip_file_type} matches File Extension {ext}')
                if zip_file_type in approved_filetypes:
                    logging.info(f'File Type {zip_file_type} is an approved Type')
                    for k,v in mime_mapping.items():
                    ...
                    logging.info(f'File Type ({zip_file_type}) is not approved.')
                    new_tags = {
                        "ERROR_STATUS": 'File Type Not Supported',
                        "MIME_TYPE": zip_mime
                    }
                ...
                logging.info(f'File Type ({zip_file_type}) does not match file extension ({ext}).')
                new_tags = {
                    "ERROR_STATUS": 'FileType does not match File Extension',
                    "MIME_TYPE": zip_mime
                }
            ...
            logging.error(f'Exception ocurred validating zipfile: {e}')
