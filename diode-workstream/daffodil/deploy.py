...
@app.command()
def roles(
    template: Annotated[typer.FileText, typer.Option(prompt="Daffodil Roles CloudFormation template path")] = "target/daffodil-roles.yaml",
    iam_prefix: Annotated[str, typer.Option(prompt="IAM Prefix")] = "Cust",
    permission_boundary: Annotated[str, typer.Option(prompt="Permission Boundary")] = "Admin"
):
    resp = cf.create_stack(
        ...
        Parameters=[
            {
                'ParameterKey': 'iamprefixparam',
                'ParameterValue': iam_prefix
            },
            {
    ...
    stack_info = cf.describe_stacks(StackName=f"daffodil-roles-{NAMESPACE}")["Stacks"][0]
    ...
    with Progress(
        ...
        while(status and not status.endswith("COMPLETE") and not status.endswith("FAILED")):
            time.sleep(2)
            stack_info = cf.describe_stacks(StackName=f"daffodil-roles-{NAMESPACE}")["Stacks"][0]
