...
import re
import jsii
from aws_cdk import aws_iam, IAspect, Stack
...
class PermissionBoundaryAspect:
    ...
    def __init__(self, *, permission_boundary: Union[aws_iam.ManagedPolicy, str], iam_prefix: str = "", iam_suffix: str = "") -> None:
        """
    ...
    def visit(self, construct_ref: IConstruct) -> None:
        ...
        def _get_role_name(resource_path: str, length: int = 128):
            ...
            hash_value = hash_value[len(hash_value) - uniqueness_len: len(hash_value)]
            role_name = re.sub("\s", "", _get_resource_id(resource_path))
            role_name = role_name[0:length - uniqueness_len - len(self.iam_prefix) - len(self.iam_suffix)]
        ...
        def _walk(obj):
            if isinstance(obj, aws_iam.Role):
                ...
                cfn_role.add_property_override('RoleName', _get_role_name(obj.node.path, 64))
            elif isinstance(obj, aws_iam.Policy):
                cfn_policy = obj.node.find_child('Resource')
                cfn_policy.add_property_override('PolicyName', _get_role_name(obj.node.path))
            elif isinstance(obj, aws_iam.ManagedPolicy):
                cfn_policy = obj.node.find_child('Resource')
                cfn_policy.add_property_override('ManagedPolicyName', _get_role_name(obj.node.path))
            elif isinstance(obj, aws_iam.InstanceProfile):
                cfn_instance_profile = obj.node.find_child('Resource')
                cfn_instance_profile.add_property_override('InstanceProfileName', _get_role_name(obj.node.path))
            else:
