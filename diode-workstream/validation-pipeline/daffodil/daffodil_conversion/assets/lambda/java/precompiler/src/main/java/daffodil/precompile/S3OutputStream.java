...
public class S3OutputStream extends OutputStream {
  ...
  public S3OutputStream(S3Client s3Client, String bucket, String path) {
    ...
    this(s3Client, bucket, path, null, null);
  }
   ...
   */
  public S3OutputStream(S3Client s3Client, String bucket, String path, String contentType, String tagging) {
    ...
    if (open) {
      ...
      if (uploadId != null) {
        ...
        if(contentType != null && !contentType.isEmpty()) {
          putRequest = putRequest.contentType(contentType);
        }
        if(tagging != null) {
          putRequest = putRequest.tagging(tagging);
        }
        ...
        RequestBody requestBody = RequestBody.fromInputStream(new ByteArrayInputStream(buf, 0, position),
            position);
        s3Client.putObject(putRequest.build(), requestBody);
      }
    }
  }
  ...
  protected void flushBufferAndRewind() {
    if (uploadId == null) {
      ...
      if(tagging != null) {
        uploadRequest = uploadRequest.tagging(tagging);
      }
      CreateMultipartUploadResponse multipartUpload = s3Client.createMultipartUpload(uploadRequest.build());
