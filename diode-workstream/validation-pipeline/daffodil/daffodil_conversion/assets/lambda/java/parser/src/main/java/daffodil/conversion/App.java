...
public class App implements RequestHandler<S3Event, Void> {
            ...
            res.exceptionally(e -> {
                ...
                if(SNS_ERROR_TOPIC_ARN != null && !SNS_ERROR_TOPIC_ARN.isBlank()) {
                    StringBuilder message = new StringBuilder(
                        String.format(
                            "An error occurred while performing daffodil parsing for file s3://%s/%s\n",
                            s3Bucket, s3Key));
                    if(moved) {
                    ...
                    try {
                        PublishRequest req = PublishRequest.builder()
                            ...
                            .build();
                        PublishResponse pub_res = snsClient.publish(req);
                        logger.info("Sent error notification to SNS: " + pub_res.messageId());
    ...
    protected boolean moveQuietly(final String srcBucket, final String destBucket, final String key) {
        ...
        try {
            ...
            logger.warn("Error occurred while moving s3://{}/{} to s3://{}/{}", srcBucket, key, destBucket, key, e.getMessage());
        }
        return false;
    }
